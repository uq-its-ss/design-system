...
⚠️ Before using the SCSS `_component.scss` file to load the icons, I would first
consider whether or not you need the entire list of icons for your application.
If you're using the Sass modules, you'll have the option to be leaner when
importing the icons you need via mixins.
...
To include _just_ the icons that you need (and therefore, reduce file size), you
can use the mixins and function provided.
...
#### Import the icon data into your own CSS class



---

##### `get-icon($icon, $color)` function

| Name | Type | Attributes | Default | Description |
| - | - | - | - | - |
| `$icon` | string | Required | N/A | String format: `{category-name}--{icon-name}` |
...
.your-class {
  background-image: url(get-icon("agriculture-and-environment--agriculture-machine-tractor"));
}
```
...
You can find a set of normalised and optimised SVG in the `dist/images` folder.
