...
Shown below are 3 different approaches in acheiving responsive behaviour in table components.

### a) Table Scroll

This is the default approach and needs to be implimented when there is lots of data. In this approach the component tables horizontal scroll is triggered when the table width is greater than the container width helping the user to view data even on smaller viewport devices. 
