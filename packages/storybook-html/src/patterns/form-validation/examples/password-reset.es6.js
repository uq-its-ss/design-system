...
var passwordResetValidation = (function() {
  ...
  function passwordResetValidation() {
    ...
    this.passwordStrengthMeterText         = document.getElementById("passwordStrengthMeterText");
    this.passwordStrengthMeterTextTemplate = document.getElementById("passwordStrengthMeterTextTemplate");
    ...
    this.passwordRegex = /^(?=.*[a-zA-Z])(?=.*[\d#$%'()*+,\-\/:;<=>\[\]^_`{|}~])(?!.*[&?!"@\\\s]).{8,40}$/;
    this.invertedAlphaRegex = /^(?!.*[a-zA-Z]).{1,}$/;
    this.invertedNonAlphaRegex = /^(?!.*[\d#$%'()*+,\-\/:;<=>\[\]^_`{|}~]).{1,}$/;
    this.invertedForbidden = /^(?=.*[&?!"@\\\s]).{1,}$/;
    this.strengthTerms = ['too guessable', 'very guessable', 'somewhat guessable', 'strong', 'very strong'];
  ...
  passwordResetValidation.prototype.setChoosePasswordFormErrors = function (joiResult) {
      ...
      joiResult.error.details.forEach((detail) => {
        ...
        if (_.has(this.choosePasswordFormErrors, key)) {
          if (_.isArray(this.choosePasswordFormErrors[key])) {
            ...
            this.choosePasswordFormErrors[key] = [this.choosePasswordFormErrors[key]];
            this.choosePasswordFormErrors[key].push(value);
          }
        } else {
  ...
  passwordResetValidation.prototype.handleChangePassword = function (key, labelElem, inputElem, errorElem) {
    return (e) => {
      // update the form state
      this.choosePasswordFormState[key] = e.target.value;
      var result = this.choosePasswordSchema.validate(this.choosePasswordFormState, {
        abortEarly: false
      });
  ...
  passwordResetValidation.prototype.handleChangePasswordPreStep = function (e) {
    ...
    while (this.passwordStrengthMeterText.firstChild) {
      this.passwordStrengthMeterText.removeChild(this.passwordStrengthMeterText.firstChild);
    }
    for (var i = 0; i < 5; i++) {
    ...
    if (e.target.value.length > 0) {
      ...
      if (clonedNode) {
        if (clonedNodeScoreElem) {
          ...
          clonedNodeScoreElem.appendChild(document.createTextNode(this.strengthTerms[result.score]));
          if (result.score <= 2) {
  ...
  passwordResetValidation.prototype.renderChoosePasswordErrors = function (errorKey, labelElem, inputElem, errorElem) {
  ...
  passwordResetValidation.prototype.renderAllInlineChoosePasswordErrors = function () {
    var itemsToRenderProcess = [
      ["password", this.passwordLabel, this.passwordInput, this.passwordError],
      ["confirmPassword", this.confirmPasswordLabel, this.confirmPasswordInput, this.confirmPasswordError],
    ];
    itemsToRenderProcess.forEach((item) => {
  ...
  passwordResetValidation.prototype.init = function () {
    // init errors
    var initErrorResults = this.choosePasswordSchema.validate(this.choosePasswordFormState, {
      abortEarly: false
    });
    this.setChoosePasswordFormErrors(initErrorResults);
    this.passwordInput.addEventListener('input', _.throttle((e) => {
      // run field validation and strength testing
      this.handleChangePasswordPreStep(e);
      this.handleChangePassword('password', this.passwordLabel, this.passwordInput, this.passwordError)(e);
    }, 1200));
    this.confirmPasswordInput.addEventListener('change', this.handleChangePassword('confirmPassword', this.confirmPasswordLabel, this.confirmPasswordInput, this.confirmPasswordError));
