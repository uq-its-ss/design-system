...
function generateChangelog(releases) {
  ...
  lines.push('');
  lines.push('All notable changes to the UQ Design System are documented in this file.');
  lines.push('');
  lines.push('This changelog aggregates stable releases only. For alpha releases and detailed per-package changelogs, see:');
  lines.push('- [GitHub Releases](https://github.com/uq-its-ss/design-system/releases) (includes alpha releases)');
  ...
  lines.push('## Archive');
  lines.push('');
  lines.push('For releases before this changelog was automated, see the [GitHub Releases page](https://github.com/uq-its-ss/design-system/releases).');
  lines.push('');
...
async function main() {
  try {
    ...
    if (releases.length === 0) {
      console.log('No stable releases found. Skipping CHANGELOG.md generation.');
      return;
    }
    ...
    console.log(`✅ CHANGELOG.md generated successfully (${releases.length} releases)`);
