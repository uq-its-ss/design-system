...
const storybookStaticDir = path.join(__dirname, '../packages/storybook-htmlstorybook-static'); // Adjust if your build directory is different
...
try {
  ...
  console.log(`Successfully generated ${outputConfigPath} with ${Object.keys(uris).length} URIs.`);
} catch (error) {
  console.error('Error generating screenshot config:', error);
  console.error('Ensure Storybook has been built and stories.json exists at:', storiesJsonPath);
